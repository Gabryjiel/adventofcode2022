...
fn task1() {
    let input =
        std::fs::read_to_string("./src/bin/day7/example.txt").expect("Unable to read the file");

    let tree: Tree<&str, u32> = Tree::new(Node::new("/", 0));

    let mut defa = String::from("/");

    let dirs = vec!["/"];

    let (new_tree, _) = input.split('\n').fold((tree, &mut defa), |acc, cur| {
        ...
        if cur.contains("$ cd ..") {
            ...
            let current_node = tree
                .root_mut()
                .query_mut(&(position.clone().as_str()))
                .expect("Unable to find current node");
