...
fn task1() {
  ...
  data
    ...
    .for_each(|x| x.filter(|(_, value)| value.is_alphabetic()).for_each(|(index, value)| crates_final[index].push(value)));
  ...
  for (amount, start, end) in commands_parsed {
    let crates_to_move = state[start][0..amount].iter().collect::<String>().chars().collect::<Vec<_>>();
...
fn task2() {
  ...
  let mut state = crates_parsed.clone();
  for (amount, start, end) in commands_parsed {
    let mut crates_to_move = state[start][0..amount].iter().collect::<String>().chars().collect::<Vec<_>>();
